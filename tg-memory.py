...
def recall(chat_id, current_message):
    ...
    for hit in results:
        ...
        lines.append("---")
        # Cross-session topic linking (PRD #29): flag strong matches from an
        # earlier session so Monet can surface "we discussed this before".
        if age_days >= 2 and hit.get("score", 0) >= 0.55 and ts:
            when = datetime.datetime.fromtimestamp(ts).strftime("%b %d")
            lines.append(f"📎 Related — you discussed this on {when} ({age_days}d ago); "
                         f"reference it naturally if relevant:")
...
def sessions(chat_id, n=3):
    """Return the last N *session episodes* (session-summary entries) only,
    newest last. Used to warm a fresh session start (PRD #49)."""
    try:
        r = requests.post(
            f"{QDRANT_URL}/collections/{COLLECTION}/points/scroll",
            headers=HEADERS,
    ...
    summaries = [p for p in points
                 if str(p.get("payload", {}).get("user_msg", "")).startswith("session-summary")]
    if not summaries:
        return ""
    summaries.sort(key=lambda p: p.get("payload", {}).get("timestamp", 0), reverse=True)
    summaries = summaries[:n]
    summaries.reverse()

    lines = [f"[Recent session recap — what you and {OWNER_NAME} worked on last]"]
    for pt in summaries:
        asst = pt.get("payload", {}).get("assistant_msg", "")
        if len(asst) > 400:
            asst = asst[:400] + "..."
        lines.append(f"• {asst}")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    if cmd == "store" and len(sys.argv) >= 5:
        ...
        if result:
            print(result)
    elif cmd == "sessions" and len(sys.argv) >= 3:
        n = int(sys.argv[3]) if len(sys.argv) >= 4 else 3
        result = sessions(sys.argv[2], n)
        if result:
            print(result)
    else:
        sys.exit(1)
