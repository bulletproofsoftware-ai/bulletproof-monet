...
# Without this, a caller with a different cwd writes/resumes sessions in a different
# slug dir, making --resume fail with "No conversation found" on a file that exists.
...
RAW=""; EC=0
run_inline "$SESSION_ID"
# Stale/missing session transcript => retry once fresh instead of surfacing an error.
# Mirrors the run_resume -> run_fresh ladder in monet-tg.sh.
if [ -n "$SESSION_ID" ] && [ "$EC" != "124" ] && { [ "$EC" != "0" ] || [ -z "${RAW:-}" ]; }; then
    log "inline resume failed ec=$EC sid=$SESSION_ID -> retrying fresh"
    run_inline ""
fi
